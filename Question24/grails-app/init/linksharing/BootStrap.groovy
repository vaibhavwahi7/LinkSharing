            ...
            User user->
                user.readingItem.each {
                    if(!it.isRead && ResourceRating.findAllByUser(user).size()==0){
                        ResourceRating resourceRating=new ResourceRating(user: user, resource: it.resource,score:4,createdBy: it.user)
